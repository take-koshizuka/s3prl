...
from .dataset import CMU_ARCTICDataset
...
class Loss(nn.Module):
    ...
    def __init__(self, stats):
        ...
        self.register_buffer("target_mean", torch.from_numpy(stats["means_"]).float())
        self.register_buffer("target_scale", torch.from_numpy(stats["scales_"]).float())

    def normalize(self, x, spk_labels):
        target_mean, target_scale = F.embedding(spk_labels, self.target_mean), F.embedding(spk_labels, self.target_scale)
        return (x - target_mean.unsqueeze(1)) / target_scale.unsqueeze(1)

    def forward(self, x, y, x_lens, y_lens, spk_labels, device):
        ...
        x_normalized = self.normalize(x, spk_labels)
        y_normalized = self.normalize(y.to(device), spk_labels)
...
class DownstreamExpert(nn.Module):
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, **kwargs):
        ...
        self.trgspks = downstream_expert['trgspks']
        ...
        scalers = { spk: None for spk in TGTSPKS }
        for spk in TGTSPKS:
            scalers[spk] = StandardScaler()
            stats_root = self.datarc["stats_root"]
            if not os.path.exists(stats_root):
                os.makedirs(stats_root)
            stats_path = os.path.join(stats_root, spk+".h5")
            if os.path.exists(stats_path):
                print("[Stats] - reading stats from " + str(stats_path))
                scalers[spk].mean_ = read_hdf5(stats_path, "mean")
                scalers[spk].scale_ = read_hdf5(stats_path, "scale")
            else:
                print("[Stats] - " + str(stats_path) + " does not exist. Calculating statistics...")
                train_dataset = CMU_ARCTICDataset('train', [spk], **self.datarc)
                for _, _, lmspc, _, _, _ in train_dataset:
                    scalers[spk].partial_fit(lmspc)
                write_hdf5(stats_path, "mean", scalers[spk].mean_.astype(np.float32))
                write_hdf5(stats_path, "scale", scalers[spk].scale_.astype(np.float32))
                print("[Stats] - writing stats to " + str(stats_path))
                del train_dataset
            
        
        self.stats = {
            "means_": np.stack([ scalers[spk].mean_ for spk in TGTSPKS ], axis=0),
            "scales_": np.stack([ scalers[spk].scale_ for spk in TGTSPKS ], axis=0)
        }
    ...
    def get_dataloader(self, split):
        if split == 'train':
            if self.train_dataset is None:
                self.train_dataset = CMU_ARCTICDataset('train', self.trgspks, **self.datarc)
            return self._get_train_dataloader(self.train_dataset)            
        elif split == 'dev':
            self.dev_dataset = CMU_ARCTICDataset('dev', self.trgspks, **self.datarc)
            return self._get_eval_dataloader(self.dev_dataset)
        elif split == 'test':
            self.test_dataset = CMU_ARCTICDataset('test', self.trgspks, **self.datarc)
    ...
    def forward(self,
        ...
        ref_spk_labels = ref_spk_labels.to(device)
        ...
        if split == "custom_test":
            ...
            if split in ["dev", "test"]:
                ...
                predicted_features, predicted_feature_lengths = self.model(input_features, input_feature_lengths, ref_spk_labels, acoustic_features_padded.to(device))
