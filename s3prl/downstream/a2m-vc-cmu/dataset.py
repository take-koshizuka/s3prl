...
class CMU_ARCTICDataset(Dataset):
    def __init__(self, split, trgspks, data_root, lists_root, fbank_config, train_dev_seed=1337, **kwargs):
        self.trgspks = trgspks
        self.fbank_config = fbank_config
        X = []
        if split == 'train' or split == 'dev':
            # number: cmu_us_{awb}_arctic/wav/
            file_list = open(os.path.join(lists_root, split + '_list.txt')).read().splitlines()
            for trgspk in trgspks:
                for number in file_list:
                    wav_path = os.path.join(data_root, f"cmu_us_{trgspk}_arctic", "wav", "arctic_" + number + ".wav")
                    if os.path.isfile(wav_path):
                        X.append((wav_path, trgspk))
            ...
            X = [ (os.path.join(data_root, f"cmu_us_{srcspk}_arctic", "wav", number + ".wav"), tgtspk) for number in file_list for srcspk in SRCSPKS for tgtspk in TGTSPKS ]
        else:
            raise ValueError('Invalid \'split\' argument for dataset: CMU ARCTIC Dataset!')
    ...
    def __getitem__(self, index):
        wav_path = self.X[index][0]
        ref_spk_name = self.X[index][1]
        ref_spk_label = TGTSPKS.index(ref_spk_name)
        ...
        return wav_resample, wav_original, lmspc, ref_spk_label, wav_path, ref_spk_name
    ...
    def collate_fn(self, batch):
        ...
        ref_spk_labels = torch.from_numpy(np.array([sorted_batch[i][3] for i in range(bs)]))
        wav_paths = [sorted_batch[i][4] for i in range(bs)]
        ref_spk_names = [sorted_batch[i][5] for i in range(bs)]
        return wavs, wavs_2, acoustic_features, acoustic_features_padded, acoustic_feature_lengths, wav_paths, ref_spk_labels, ref_spk_names, None
